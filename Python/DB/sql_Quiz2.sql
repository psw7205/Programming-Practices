...
SELECT dept_no, COUNT(*) as cnt FROM dept_emp WHERE to_date = '9999-01-01' GROUP BY dept_no ORDER BY cnt DESC;

-- 5. 부서 이동이 많은 직원순으로 리스트를 출력하시오 (퇴직자 포함)

SELECT emp_no, COUNT(*) as CNT FROM dept_emp GROUP BY EMP_NO HAVING COUNT(*)>1 ORDER BY CNT DESC

-- 6. 부서별 현재 인원수가 15,000명 이상인 부서를 구하시오.

SELECT dept_no, COUNT(*) cnt FROM dept_emp WHERE to_date = '9999-01-01' GROUP BY dept_no HAVING COUNT(*) >= 15000 ORDER BY cnt DESC;

-- 7. 부서별 현재 인원수가 가장 많은 상위 5개 부서를 구하시오

SELECT dept_no, COUNT(*) cnt FROM dept_emp WHERE to_date = '9999-01-01' GROUP BY dept_no ORDER BY cnt DESC LIMIT 5;
